...
class SafeSoftmaxFunction(Function):
    ...
    def forward(ctx, x, dim, dtype):
        ...
        if dtype is None:
            dtype = x.dtype
        out = genesis.empty_like(x, dtype=dtype, requires_grad=x.requires_grad)
        K = inp.numel() // M // N
...
@triton.heuristics({
    ...
    "ONE_TILE_PER_CTA": heur_one_tile_per_cta, "num_warps": heur_num_warps_non_inner,
})
@triton.jit
...
def _online_softmax_backward_kernel_inner(
    ...
    if ONE_TILE_PER_CTA:
        ...
        out_tile = tl.load(out_ptr + offsets, mask=mask, other=0.0)
