...
class MultiheadAttention(Module):
    ...
    def __init__(
        ...
        self.w_qkv = Parameter(
            init.kaiming_uniform(self.dim, self.dim * 3, device=device, dtype=dtype))
        self.w_out = Parameter(
            init.kaiming_uniform(self.dim, self.dim, device=device, dtype=dtype))
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        q, k, v = [
            a.reshape(x.shape[0], x.shape[1], self.heads, head_dim).transpose(1, 2)
            for a in [q, k, v]
        ]
        mask = genesis.triu((-float("inf") * init.ones(x.shape[1], x.shape[1], device=x.device)), k=1, device=x.device)
        atten = self.softmax(q @ F.transpose(k, axis=(-1, -2)) / np.sqrt(self.dim // self.heads) + mask)
